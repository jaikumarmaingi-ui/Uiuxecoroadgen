...
function StatusPill({ label, icon: Icon }: { label: string; icon: typeof Wifi }) {
  return (
    <div className="hidden items-center gap-1.5 rounded-full border border-green/25 bg-green/[0.06] px-2.5 py-1 text-[10px] font-semibold tracking-wide text-green lg:flex">
...
export function TopNav({ onMenuClick }: { onMenuClick: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex items-center gap-3 border-b border-hairline bg-base/85 px-4 py-3 backdrop-blur md:px-6">
      ...
      <div className="hidden items-center gap-2 rounded-lg border border-hairline-strong bg-white/[0.03] px-3 py-1.5 text-xs text-text-secondary sm:flex">
      ...
      <div className="relative">
        <button
          ...
          className="flex items-center gap-2 rounded-lg border border-hairline-strong bg-white/[0.03] py-1.5 pl-1.5 pr-2.5 hover:border-cyan/30"
        >
